...
**当前为本地审计修复工作树，不等同于 GitHub 已发布版本。**本地 `HEAD` 与 GitHub `main` 均为
`9a7d02d`，但本地仍有未提交修改和未跟踪审计/结果文件；核心候选集统计可由本地最终 tier 表复算，
