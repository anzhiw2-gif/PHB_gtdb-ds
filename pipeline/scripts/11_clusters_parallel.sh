...
while IFS=$'\t' read -r batch genomes loci; do
  ...
  if [ ! -s "$root/rc" ] || ! grep -qx '0' "$root/rc"; then
    rm -f "$root/rc"
    setsid -f bash -c "$PY $SCRIPT_DIR/11_clusters.py --hits $root/hits.tsv --marker-hmms data/hmms/v2 --marker-families $MARKERS --gtdb /home/data/haoyu/GTDB/gtdb_genomes_reps_r232/database --outdir $root/results --workdir $root/data/work --flank-kb 10 --threads 1 --max-genomes 0 > $root/results/logs/cluster.log 2>&1; printf '%s\\n' \$? > $root/rc" </dev/null
  fi
