...
## 六、PhaDED 架构扫描与多证据筛选（2026-09-09 → 2026-09-19）

### 版本信息

- **方法升级**：从 9 家族 HMM（run-13）升级为 **PhaDED 架构 HMM**——Knoll 2009 的
  **8 个超家族 + 38 个家族**。流程 = 先用 PhaDED 架构 HMM 全库扫描，再对命中序列施加
  多证据筛选（InterPro 域证据 / SignalP 定位 / 催化域架构 / 强 profile 得分 / 唯一归属 /
  排除 x₁ 非疏水混淆）。
- **候选宇宙**：池内 109,087 条；多证据高可信度筛选后池内 36,611 条 + 池外（仅 profile 得分）
  35,558 条。
- **校准 gate（fail-closed）**：每家族需 ≥3 条独立实验阳性才可校准；截至 2026-09-19
  **无一家族达标**（Cys 型最多 2 条：PhaZ1 + PhaZ2）。因此未发布任何校准 profile。
- **边界**：全部产物为 candidate-only，不等于已验证 PHB/PHA 降解表型；Cys 型为**推定** Cys 型
  （催化残基未逐条验证）。

### 运维教训（供后续会话避免）

1. **元数据优先于内容 grep**：先读 manifest/schema，再按内容搜索，避免同名文件跨 run 混用。
2. **全库试点用中位分片**：避免用最大/最小分片做代表，造成偏倚。
3. **上传清单先去 CRLF**：Windows 换行在 Linux 侧解析会出错。
4. **`xargs -n 1` 下 `{}` 不替换**：需用 `-I{}`。
5. **内联 ssh 双引号丢失**：远端 `>` 被当 shell 重定向而截断文件——服务器操作一律写成脚本文件
   再执行，禁止内联拼字符串。
